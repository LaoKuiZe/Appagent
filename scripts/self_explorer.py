...
if not os.path.exists(demo_dir):
    os.mkdir(demo_dir)
demo_timestamp = int(time.time()) # 生成demo文件的时间戳
...
while round_count < configs["MAX_ROUNDS"]:
    ...
    elem_list = []
    for elem in clickable_list: # 遍历所有可点击的UI，如果已经在useless_list中，则跳过
        if elem.uid in useless_list:
            continue
        elem_list.append(elem) # 否则加入elem_list
    for elem in focusable_list: # 如果是focusable的UI，也加入elem_list
        if elem.uid in useless_list:
            continue
        bbox = elem.bbox
    ...
    # 替换后的 prompt 就是一个包含实际任务信息和上次动作的字符串，用于引导 AI 模型判断下一步如何执行
    ...
    status, rsp = mllm.get_model_response(prompt, [base64_img_before]) # status表示是否成功，rsp是返回的结果
